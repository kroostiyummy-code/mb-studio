...
| `audit-livraison` | ✅ Fait (SKILL.md + SPEC + 3 références + registre-differenciation + 2 templates) | Audit du site juste avant livraison. Produit 2 outputs : audit interne brut (Mike-only) + rapport de mise en service valorisant (patron). Compare au snapshot eatbu du brief + moyenne sectorielle anonyme. **Étape 7bis garde-fou anti-jumeau** (registre des combinaisons livrées, bloque un site trop proche d'un livré même ville — principe #10). |
...
| `scoring-prospects` | ✅ Fait (SKILL.md + SPEC + 3 références + 2 templates) | Génère 3 listes triées (avec site eatbu / avec site autre / sans site) de prospects chartrains à démarcher, scoring 0-100 + tier A/B/C combinant "valeur apportée" et "probabilité d'accepter". **Source de vérité = liste vivante `prospects/restaurants.yml`** (vues régénérées : CSV plat + snapshot daté avec top 3 arguments). Sources données : Overpass OSM + PageSpeed + Wayback + **Google Places API officielle**. |
| `pilote-client` | ✅ Fait (SKILL.md + SPEC + gates + messages-tunnel + carnet-vide) — **point d'entrée central** | Méta-skill chef d'orchestre. Pilote Mike comme un tunnel du scoring au suivi J+30 : une seule action affichée à la fois, gates bloquantes (pas de prod sans acompte, pas de livraison sans audit vert), carnet `pilote/{slug}.yml` auto-maintenu + **miroir du bloc `tunnel` dans `prospects/restaurants.yml`**, suivi des délais avec cadeau "livraison anticipée" structurel. Orchestre tous les autres skills sans réimplémenter leur logique. |
...
- **État au 2026-05-16** : template `site-resto/` ✅ (3 signatures renommées **fast-food / gastro / traditionnel**, polices 100% auto-hébergées, marqueur d'emphase volontaire). **TOUS les skills sont écrits et réconciliés** (y compris `audit-livraison` avec garde-fou anti-jumeau étape 7bis). `scoring-prospects` et `pilote-client` branchés sur la **liste vivante `prospects/restaurants.yml`** (source de vérité unique). **Prochains jalons** : (1) régulariser le statut juridique (activité secondaire BIC) avant 1ʳᵉ facture, (2) vrai run `scoring-prospects` Chartres + reprise La Casa Tropical via le nouveau tunnel, (3) plus tard : vitrine MB Studio (pas avant 3ᵉ client signé).
- ~~**Prochain jalon Mike** : construire le template puis les skills AVANT le terrain~~ ✅ fait (template + skills prêts ; Mike peut arriver "lancé").
