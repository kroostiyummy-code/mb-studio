...
- **État au 2026-05-16** : template `site-resto/` ✅ (3 signatures renommées **fast-food / gastro / traditionnel**, polices 100% auto-hébergées, marqueur d'emphase volontaire). **Tous les skills sont écrits** sauf `audit-livraison` (spec seule). `scoring-prospects` et `pilote-client` ✅ et branchés sur la **liste vivante `prospects/restaurants.yml`** (source de vérité unique). **Prochains jalons** : (1) coder `audit-livraison`, (2) régulariser le statut juridique avant 1ʳᵉ facture, (3) vrai run `scoring-prospects` Chartres + reprise La Casa Tropical via le nouveau tunnel.
